...
class TestAgentCreation:
    def test_conversation_agent(self):
        agent = create_conversation_agent(model="gemini-3-flash-preview")
        assert agent.name == "conversation_agent"
        assert len(agent.tools) == 2

    def test_reflection_agent(self):
        agent = create_reflection_agent(model="gemini-3-flash-preview")
        assert agent.name == "reflection_agent"
        assert len(agent.tools) == 4

    def test_predictive_agent(self):
        agent = create_predictive_agent(model="gemini-3-flash-preview")
